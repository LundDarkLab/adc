...
class Institution extends Conn{
  ...
  function __construct(){
    ...
    $this->logoFolder = $_SERVER['DOCUMENT_ROOT']."/archive/logo/";
  }
  ...
  public function getInstitution(int $id){
    $sql="select i.is_storage_place, i.color, i.category catid, cat.value category, i.name, i.abbreviation, i.city, i.address, i.lat, i.lon, i.url, i.logo FROM institution i INNER JOIN list_institution_category cat ON i.category = cat.id where i.id = ".$id.";";
  ...
  public function editInstitution(array $dati, $file){
    try {
      ...
      if(count($file) > 0){
        ...
        if ($logo[0]['logo'] && $logo[0]['logo'] !== 'default.png') {
