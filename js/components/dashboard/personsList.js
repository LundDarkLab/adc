...
export async function getPersonsList(filters={}) {
  const isLogged = document.getElementById('isLogged').value;
  const userId = Number.parseInt(document.getElementById('userId').value);
  const userRole = Number.parseInt(document.getElementById('userRole').value);
  ...
  list.forEach(person => {
    ...
    viewBtn.href = `person_view.php?item=${person.person_id}`;
    cardFooter.appendChild(viewBtn);

    const editBtn = document.createElement('a');
    editBtn.classList.add('btn', 'btn-sm', 'btn-adc-blue', 'ms-2');
    editBtn.textContent = 'Edit';
    editBtn.href = `person_edit.php?item=${person.person_id}`;
    if(isLogged && (userId === person.id || userRole === 1)){
