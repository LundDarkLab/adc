 ...
 * @param {string} viewerContext.firstMesh - chiave della prima mesh in modelInstances
...
export function initAnnotations(viewerContext, annotationsContext){
  const { presenter, viewerState, viewerEl, measureTool, setViewerState, firstMesh } = viewerContext;
  ...
  function displaySpots() {
    ...
    // Raggio dello spot relativo alla dimensione reale della scena, non assoluto:
    // la sphere.ply è unitaria e lo scale era fisso (2.5 unità modello), quindi su
    // modelli piccoli risultava enorme e su modelli con coordinate molto grandi
    // (es. unità interne di RealityCapture) spariva. Lo leghiamo al raggio scena.
    // NB: sceneRadiusInv è calcolato da 3DHOP solo dentro il draw (non a scene-ready):
    // al primo display potrebbe valere ancora il default 1.0 → spot microscopici e
    // invisibili in modo intermittente al reload. Forziamo il calcolo ora.
    if (typeof presenter._setSceneCenterRadius === 'function') presenter._setSceneCenterRadius();
    const SPOT_RADIUS_FACTOR = 0.02; // ~2% del raggio della scena
    const sceneRadius = 1 / presenter.sceneRadiusInv;

    for (let spot in viewerAnnotations.spots) {
      const radius = sceneRadius * SPOT_RADIUS_FACTOR;
      const opoint = [
      ...
      const tpoint = SglMat4.mul4(presenter._scene.modelInstances[firstMesh].transform.matrix, opoint);
