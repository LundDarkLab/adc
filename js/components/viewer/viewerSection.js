  ...
  function sectionListener(event) {      
    ...
    [...planeToggles].forEach(toggle => {
      toggle.addEventListener('click', function() {
        ...
        const index = planeToIndex(plane);
    ...
    [...planeRanges].forEach(range => {
      range.addEventListener('input', function() {
        ...
        VIEWER_STATE.clippingPoint[index] = Number.parseFloat(this.value);
        setSections();
      });
    });
    ...
    if (sectionTool && sectionTool.length > 0) {
      for (const element of sectionTool) {
