...
function createMaterialTechniqueRow(m, matTxt, t, matTechArrayEl) {
...
export function handleMaterialTechnique(ev) {
  ...
  const checkM = materialTechniqueArray.some(item => parseInt(item.m) === m);
  if (checkM) {
  ...
  const row = createMaterialTechniqueRow(m, matTxt, t, matTechArrayEl);
...
export function populateMaterialTechniqueFromArray(dataArray) {
  const matTechArrayEl = document.getElementById('matTechArray');
  if (!matTechArrayEl) {
    console.error('matTechArray element not found');
    return false;
  }

  // Popola materialTechniqueArray mappando i dati in ingresso
  materialTechniqueArray = dataArray.map(item => ({
    m: item.material_id,
    t: item.technique
  }));

  // Svuota il DOM esistente
  matTechArrayEl.innerHTML = '';

  // Per ogni elemento nell'array, crea la riga DOM usando il testo del materiale dall'array in ingresso
  dataArray.forEach((item) => {
    const row = createMaterialTechniqueRow(item.material_id, item.material, item.technique, matTechArrayEl);
    matTechArrayEl.appendChild(row);
  });
  return true;
}
