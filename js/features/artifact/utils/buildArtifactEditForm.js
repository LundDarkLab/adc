...
function handleFindPlaceField(data){
  if(data.parish){ 
    const parish = document.getElementById('parish');
    if(parish) parish.value = data.parish;
    delete data.parish;
  }
  if(data.toponym){ 
    const toponym = document.getElementById('toponym');
    if(toponym) toponym.value = data.toponym;
    delete data.toponym;
  }
  if(data.notes){ 
    const notes = document.getElementById('findplace_notes');
    if(notes) notes.value = data.notes;
    delete data.notes;
  }
}

function handleLatLon(latitude,longitude, mapInit){
...
async function fitMapToBoundary(mapInit, maxLevel, bounds) {
  if (maxLevel < 0) {
  ...
  try {
    ...
    if (response.error === 0 && response.data?.items?.length > 0) {
      const geoJson = JSON.parse(response.data.items[0].geom);
      mapInit.map.fitBounds(L.geoJSON(geoJson).getBounds());
    } else {
...
async function setFindPlaceSection(data, mapInit) {
  ...
  const maxLevel = Object.keys(bounds).length > 0
    ? Math.max(...Object.keys(bounds).map(k => parseInt(k)))
    : -1;
  ...
  for (const [level, gid] of Object.entries(bounds)) {
    ...
    document.getElementById(`gid_${nextLevel}_container`)?.classList.remove('hide');
  }

  if (data.latitude && data.longitude) {
    handleLatLon(data.latitude, data.longitude, mapInit);
  } else {
    await fitMapToBoundary(mapInit, maxLevel, bounds);
  }
}
...
function setMainDataSection(data) {
  ...
  const checked = data['is_museum_copy'] === 1;
  copyCheckbox.checked = checked;
  copyCheckbox.setAttribute('aria-checked', checked);
