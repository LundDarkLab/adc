...
// Utility: rimuove un layer dalla mappa e lo svuota se possibile
...
// Utility: pulisce tutti i gruppi della collectionGroup
function clearCollectionGroups(mapElement) {
  for (const key in mapElement.collectionGroup) {
    removeLayer(mapElement.map, mapElement.collectionGroup[key]);
  }
  mapElement.collectionGroup = {};
}

// Utility: pulisce il findplace layer dal layerControl e dal gruppo separato
function clearFindplaceLayers(mapElement) {
  const existingLayer = mapElement.layerControl?.poi?.findplace?.layer;
  removeLayer(mapElement.map, existingLayer);

  if (mapElement.findPlaceGroup && mapElement.findPlaceGroup !== existingLayer) {
    removeLayer(mapElement.map, mapElement.findPlaceGroup);
  }
...
async function performRefresh(mapElement, options = {}) {
  clearCollectionGroups(mapElement);
  clearFindplaceLayers(mapElement);
