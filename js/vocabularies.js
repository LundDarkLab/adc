...
listTable.appendChild(listFragment);
listTableSelect.appendChild(selectFragment);

// La pagina è pronta: nascondi l'overlay di caricamento.
    ...
    arrayKeys.forEach(k => {
      ...
      if (obj.hasOwnProperty(k)) {
        ...
        if (k === 'category_class') {
          ...
          select.dataset.name = 'category_class';
    ...
    arrayKeys.forEach(k => {
      ...
      if (obj.hasOwnProperty(k)) {
        ...
        if (k === 'material_class') {
          const select = document.createElement('select');
          select.className = `form-select form-select-sm ${list}_input_${i}`;
          select.dataset.name = 'material_class';
          select.required = true;
          data.lists.forEach(item => {
            const option = document.createElement('option');
            option.value = item.id;
...
function deleteVocabularyItem(table,id){
  deleteModal.show();
  // Sostituisce il bottone con un clone per azzerare i listener accumulati
  // dalle aperture precedenti del modal.
  const oldBtn = document.getElementById('deleteConfirmButton');
  const delBtn = oldBtn.cloneNode(true);
  oldBtn.replaceWith(delBtn);
